...
if __name__ == '__main__':
    ...
    for frict in frict_vals:
        for slope in slope_vals:
            ...
            with open('../configurations/sysidEnvParams.yaml') as config:
                temp = yaml.safe_load(config)
                temp['terrainParams']['slope'] = float(slope)
                temp['terrainParams']['setFriction'] = float(frict)

            with open('../configurations/sysidEnvParams.yaml', 'w') as config:
                yaml.dump(temp, config)


            env = WheeledSimEnv(config_file, T=50, render=False)

            step_funcs = generate_step_functions(T, THROTTLE_N, STEER_N)
